    ...
    const int requestedTests)
{
	...
	if (plannedTests < opt::min_tests) {
		plannedTests = opt::min_tests;
	}
	if (plannedTests > opt::min_tests + MAX_TESTS_OFFSET) {
		return Support();
	}
...
determinePathSupport(const ContigPath& path)
{
	...
	const long calculatedTests =
	    std::round(expectedSpacingBetweenReads(path) * COV_APPROX_FORMULA_FACTOR);
	assert(calculatedTests >= 0);

	long requiredTests = calculatedTests;
	if (requiredTests < opt::min_tests) {
		requiredTests = opt::min_tests;
	}
...
#pragma omp critical(resolution)
			    {
				    ...
				    updateStats(resolution, pathsKnown, pathsUnknown, supportMap, inHistSample);
			    }
		    }
